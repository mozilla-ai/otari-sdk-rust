...
- **Generated core: the `_client` module in `src/_client/`**: produced by OpenAPI Generator from
  the gateway's OpenAPI spec. It is **generated, not hand-edited.** Regeneration happens upstream
  in the gateway repo (`.github/workflows/otari-sdk-codegen.yml`), which opens a
  `sdk-codegen/client-core` PR here. The core is a module of this crate rather than a separate
  path dependency, so `cargo publish` ships it with no extra step; see [RELEASE.md](RELEASE.md).
...
- [RELEASE.md](RELEASE.md): How a release is cut and published.
...
- Never hand-edit the generated `src/_client/` module; it is regenerated from the gateway spec.
