...
pub async fn create_speech_v1_audio_speech_post(configuration: &configuration::Configuration, audio_speech_request: models::AudioSpeechRequest) -> Result<serde_json::Value, Error<CreateSpeechV1AudioSpeechPostError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CreateSpeechV1AudioSpeechPostError> = serde_json::from_str(&content).ok();
...
pub async fn create_transcription_v1_audio_transcriptions_post(configuration: &configuration::Configuration, file: std::path::PathBuf, model: &str, language: Option<&str>, prompt: Option<&str>, response_format: Option<&str>, temperature: Option<f64>, user: Option<&str>) -> Result<serde_json::Value, Error<CreateTranscriptionV1AudioTranscriptionsPostError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CreateTranscriptionV1AudioTranscriptionsPostError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
