...
pub async fn create_response_v1_responses_post(configuration: &configuration::Configuration, responses_request: models::ResponsesRequest) -> Result<serde_json::Value, Error<CreateResponseV1ResponsesPostError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CreateResponseV1ResponsesPostError> = serde_json::from_str(&content).ok();
