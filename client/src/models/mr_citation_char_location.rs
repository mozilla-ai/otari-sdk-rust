/*
 * otari-gateway
 *
 * A clean FastAPI gateway for otari with API key management
 *
 * The version of the OpenAPI document: 0.0.0-dev
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct MrCitationCharLocation {
    #[serde(rename = "cited_text")]
    pub cited_text: String,
    #[serde(rename = "document_index")]
    pub document_index: i32,
    #[serde(rename = "document_title", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub document_title: Option<Option<String>>,
    #[serde(rename = "end_char_index")]
    pub end_char_index: i32,
    #[serde(rename = "file_id", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub file_id: Option<Option<String>>,
    #[serde(rename = "start_char_index")]
    pub start_char_index: i32,
    #[serde(rename = "type")]
    pub r#type: Type,
}

impl MrCitationCharLocation {
    pub fn new(cited_text: String, document_index: i32, end_char_index: i32, start_char_index: i32, r#type: Type) -> MrCitationCharLocation {
        MrCitationCharLocation {
            cited_text,
            document_index,
            document_title: None,
            end_char_index,
            file_id: None,
            start_char_index,
            r#type,
        }
    }
}
/// 
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Type {
    #[serde(rename = "char_location")]
    CharLocation,
}

impl Default for Type {
    fn default() -> Type {
        Self::CharLocation
    }
}

