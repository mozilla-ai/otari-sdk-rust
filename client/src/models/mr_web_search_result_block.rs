/*
 * otari-gateway
 *
 * A clean FastAPI gateway for otari with API key management
 *
 * The version of the OpenAPI document: 0.0.0-dev
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct MrWebSearchResultBlock {
    #[serde(rename = "encrypted_content")]
    pub encrypted_content: String,
    /// Filter models by provider name
    #[serde(rename = "page_age", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub page_age: Option<Option<String>>,
    #[serde(rename = "title")]
    pub title: String,
    #[serde(rename = "type")]
    pub r#type: Type,
    #[serde(rename = "url")]
    pub url: String,
}

impl MrWebSearchResultBlock {
    pub fn new(encrypted_content: String, title: String, r#type: Type, url: String) -> MrWebSearchResultBlock {
        MrWebSearchResultBlock {
            encrypted_content,
            page_age: None,
            title,
            r#type,
            url,
        }
    }
}
/// 
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Type {
    #[serde(rename = "web_search_result")]
    WebSearchResult,
}

impl Default for Type {
    fn default() -> Type {
        Self::WebSearchResult
    }
}

