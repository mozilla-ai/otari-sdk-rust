...
/// struct for typed errors of method [`audio_create_speech`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum AudioCreateSpeechError {
    Status422(models::HttpValidationError),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`audio_create_transcription`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum AudioCreateTranscriptionError {
    Status422(models::HttpValidationError),
    UnknownValue(serde_json::Value),
}
...
pub async fn audio_create_speech(
    configuration: &configuration::Configuration,
    audio_speech_request: models::AudioSpeechRequest,
) -> Result<serde_json::Value, Error<AudioCreateSpeechError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_audio_speech_request = audio_speech_request;

    let uri_str = format!("{}/api/v1/audio/speech", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<AudioCreateSpeechError> = serde_json::from_str(&content).ok();
...
pub async fn audio_create_transcription(
    configuration: &configuration::Configuration,
    file: &str,
    model: &str,
    language: Option<&str>,
    ...
    user: Option<&str>,
) -> Result<serde_json::Value, Error<AudioCreateTranscriptionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    let uri_str = format!("{}/api/v1/audio/transcriptions", configuration.base_path);
    let mut req_builder = configuration
        .client
        .request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<AudioCreateTranscriptionError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
