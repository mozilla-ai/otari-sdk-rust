...
/// struct for typed errors of method [`bootstrap_get_bootstrap`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum BootstrapGetBootstrapError {
...
pub async fn bootstrap_get_bootstrap(
    configuration: &configuration::Configuration,
) -> Result<models::DeploymentBootstrap, Error<BootstrapGetBootstrapError>> {
    let uri_str = format!("{}/api/v1/bootstrap", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<BootstrapGetBootstrapError> = serde_json::from_str(&content).ok();
