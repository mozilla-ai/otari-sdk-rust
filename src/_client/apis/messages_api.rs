...
/// struct for typed errors of method [`messages_count_message_tokens`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum MessagesCountMessageTokensError {
    Status422(models::HttpValidationError),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`messages_create_message`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum MessagesCreateMessageError {
    Status422(models::HttpValidationError),
    UnknownValue(serde_json::Value),
}
...
pub async fn messages_count_message_tokens(
    configuration: &configuration::Configuration,
    count_tokens_request: models::CountTokensRequest,
) -> Result<models::CountTokensResponse, Error<MessagesCountMessageTokensError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_count_tokens_request = count_tokens_request;

    let uri_str = format!("{}/api/v1/messages/count_tokens", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<MessagesCountMessageTokensError> = serde_json::from_str(&content).ok();
...
pub async fn messages_create_message(
    configuration: &configuration::Configuration,
    messages_request: models::MessagesRequest,
) -> Result<models::MessageResponse, Error<MessagesCreateMessageError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_messages_request = messages_request;

    let uri_str = format!("{}/api/v1/messages", configuration.base_path);
    let mut req_builder = configuration
        .client
        .request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<MessagesCreateMessageError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
