...
/// struct for typed errors of method [`images_create_image`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ImagesCreateImageError {
...
pub async fn images_create_image(
    configuration: &configuration::Configuration,
    image_generation_request: models::ImageGenerationRequest,
) -> Result<models::ImagesResponse, Error<ImagesCreateImageError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_image_generation_request = image_generation_request;

    let uri_str = format!("{}/api/v1/images/generations", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ImagesCreateImageError> = serde_json::from_str(&content).ok();
