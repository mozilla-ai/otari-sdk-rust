...
/// struct for typed errors of method [`responses_create_response`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ResponsesCreateResponseError {
...
pub async fn responses_create_response(
    configuration: &configuration::Configuration,
    responses_request: models::ResponsesRequest,
) -> Result<serde_json::Value, Error<ResponsesCreateResponseError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_responses_request = responses_request;

    let uri_str = format!("{}/api/v1/responses", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ResponsesCreateResponseError> = serde_json::from_str(&content).ok();
