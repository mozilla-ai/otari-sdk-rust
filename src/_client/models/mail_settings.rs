/*
 * otari
 *
 * Otari, an OpenAI-compatible LLM gateway with API key management
 *
 * The version of the OpenAPI document: 0.6.4
 *
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// MailSettings : What this deployment can send, and what stands in the way if it cannot.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct MailSettings {
    /// Whether a transport is configured at all.
    #[serde(rename = "enabled")]
    pub enabled: bool,
    /// The 'From' address on outgoing mail, if one is configured.
    #[serde(rename = "from_email", deserialize_with = "Option::deserialize")]
    pub from_email: Option<String>,
    /// The 'From' display name on outgoing mail.
    #[serde(rename = "from_name")]
    pub from_name: String,
    /// Settings that must be set before mail works, in config order. Empty exactly when 'ready' is true, so the dashboard can name what to configure rather than only reporting that mail is off.
    #[serde(rename = "missing")]
    pub missing: Vec<String>,
    /// This deployment's own externally-reachable URL, which mail needs before it can send. The links themselves are built from ui_base_url, or from this when that is unset.
    #[serde(rename = "public_base_url", deserialize_with = "Option::deserialize")]
    pub public_base_url: Option<String>,
    /// Whether a message carrying a link back to this deployment can be sent, which is what every message the control plane sends needs. Matches 'mail_ready' on /api/v1/bootstrap.
    #[serde(rename = "ready")]
    pub ready: bool,
    /// The transport a send would use: 'smtp', 'console' (logged, not delivered), or 'none'.
    #[serde(rename = "transport")]
    pub transport: String,
}

impl MailSettings {
    /// What this deployment can send, and what stands in the way if it cannot.
    pub fn new(
        enabled: bool,
        from_email: Option<String>,
        from_name: String,
        missing: Vec<String>,
        public_base_url: Option<String>,
        ready: bool,
        transport: String,
    ) -> MailSettings {
        MailSettings {
            enabled,
            from_email,
            from_name,
            missing,
            public_base_url,
            ready,
            transport,
        }
    }
}
