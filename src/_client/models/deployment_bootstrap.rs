...
pub struct DeploymentBootstrap {
    ...
    /// Whether this deployment accepts deliberate feedback submissions.
    #[serde(rename = "feedback_enabled")]
    ...
    /// Whether POST /api/v1/auth/signup creates an account for an address nobody has added yet, each with an organization of its own, or only lets an address an admin already put on the roster set its password. The signup page reads as registration or as claiming an invitation accordingly, and the sign-in screen links to it with the wording that matches. False for a hybrid gateway, which holds no identities.
    #[serde(rename = "open_signup")]
    ...
    /// Whether the model catalog is served to a visitor with no session: GET /api/v1/catalog/models answers anonymously and the dashboard renders Models ahead of sign-in. False for a hybrid gateway, which serves no catalog of its own.
    #[serde(rename = "public_catalog", skip_serializing_if = "Option::is_none")]
    pub public_catalog: Option<bool>,
    ...
    #[serde(rename = "site_url", deserialize_with = "Option::deserialize")]
